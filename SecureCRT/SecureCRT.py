...
def xmodem_upload_wait_for_finish(scr,crt, cmd_to_start_upload,string_to_wait_before_upload,delay_before_upload,file_to_upload, string_to_wait_for, time_to_wait_after_string ):
	...
	reswait(scr, string_to_wait_before_upload)
	...
	time.sleep(1)
	scr.Synchronous = True
	reswait(scr, string_to_wait_for)
	scr.Synchronous = False
...
def read_from_file_and_send(file,crt):
	...
	for line in open(file, "r"):
		...
		#
		crt.Screen.WaitForString(line + '\r')

	scr.Synchronous = False

def reswait(scr, resstr):
    return scr.WaitForString(resstr)


def get_info_with_line_starting_with(scr,cmd_to_query,start_of_parameters_array):
	scr.clear() #clear the screen so we can start with a clean screen and capture everything in it
	sync_mem = scr.Synchronous
	scr.Synchronous = True
	cmd(scr, 0.5, cmd_to_query, 0.0)
	all_on_screen : str =scr.get()
	results=[] #Initialize our return array
	for params in start_of_parameters_array : # we will loop through our parameters that we want to get from our previous query
		results.append(re.findall("(?<="+params+")",scr.get())) #Appending the parameters to the list
	scr.Synchronous = False
