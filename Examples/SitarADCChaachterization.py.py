...
for temp in range(-20,70,5):
    ...
    #sweep main voltage
    main_volt_adc_1.append([temp,sweep(sitar1.get_main_volt_adc, psu.read_voltage,[1], psu.set_voltage, [1], 3, 20.5, .1, 1)])
    main_volt_adc_2.append([temp,sweep(sitar2.get_main_volt_adc, psu.read_voltage,[1], psu.set_voltage, [1], 3, 20.5, .1, 1)])
    #sweep  solar voltage
    solar_volt_adc_1.append([temp,sweep(sitar1.get_solar_volt_adc, psu.read_voltage,[2], psu.set_voltage, [2], 0, 10.5, .1, 1)])
    solar_volt_adc_2.append([temp,sweep(sitar2.get_solar_volt_adc, psu.read_voltage,[2],psu.set_voltage, [2], 0, 10.5, .1, .1)])
    #temp logging
